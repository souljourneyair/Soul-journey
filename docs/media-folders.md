...
```
public/uploads/logo/
  default.png   основной, показывается в шапке
  small.png     компактный (необязательно), для экранов уже 640 px
```

Расширение любое. Если `default` нет, подхватится первая картинка в папке —
логотип заработает, даже если файл назвали как попало. Если нет `small`,
на узких экранах показывается основной.
