...
// Упрощение MVP: каждое здание — одна отдельная постройка
// (в дизайн-документе ВПП/терминалы многоклеточные — добавим позже).
...
const PASSENGER_ECONOMY = {
  ...
  VISITOR_SPEND: 12,
  // Опыт даёт каждый десятый посетитель. За каждого выходило бы 3 600 XP
  // в реальный час с малого кафе — больше, чем все остальные источники
  // вместе, и прогресс перестал бы зависеть от развития аэропорта.
  VISITORS_PER_XP: 10,
...
// Модель с возвратом к базе. Цена меняется каждый игровой час:
//   • небольшой откат к базовой (REVERT_PER_HOUR) — чем дальше цена ушла,
//     тем сильнее тянет назад;
//   • случайное биение вокруг базы (HOURLY_NOISE).
// Серьёзное ЧС даёт разовый скачок вверх (+SHOCK_MIN..SHOCK_MAX), который
// затем плавно рассасывается откатом. Так на графике видны и рост, и спад,
// и цена не залипает у потолка (раньше ЧС били слишком часто и только вверх).
const MARKET_ECONOMY = {
  HOUR_TICKS: 60,          // игровой час — шаг обновления цены
  REVERT_PER_HOUR: 0.03,   // доля разрыва с базой, закрываемая за час
  HOURLY_NOISE: 0.006,     // случайное биение ±0.6% от базы за час
  // Мёртвая зона для экономики: отклонение цены меньше 1% не влияет.
  DEAD_ZONE: 0.01,
  // Серьёзное ЧС: рост на 3-7% по затронутым товарам.
  SHOCK_MIN: 0.03,
  SHOCK_MAX: 0.07,
  // Границы цены в долях от базы.
  OIL_MIN: 0.5, OIL_MAX: 2.0,
  GOLD_MIN: 0.5, GOLD_MAX: 2.0,
  // Сколько часовых точек истории храним: игровой год. Фильтры на графике
  // (день/неделя/месяц/год) просто отрезают нужное окно от этой истории.
  HISTORY_MAX: 365 * 24,
};
...
const DAMAGE_ECONOMY = {
  ...
  // иначе освободить место было бы нельзя при пустом кошельке.
...
const BANK = {
  MIN_LEVEL: 5,         // банк открывается с этого уровня аэропорта
  MIN_RATE: 0.17,
...
const RATING = {
  ...
  // Приток пассажиров у ТЕРМИНАЛОВ считается отдельной шкалой. Общая была
  // настроена под вертолёт на 8 мест, а самолёт возит от 50 до 310: терминал
  // порождал в 15 раз меньше людей, чем способны увезти борта, и самолёты
  // летали почти пустыми.
  //
  // Шкала подобрана так, чтобы терминал первого уровня при хорошем рейтинге
  // наполнял пару договоров, а прокачанный требовал уже строить следующий
  // терминал — это и есть задуманная лестница: A наполнился, стоянок впритык,
  // строим B, потом C.
  TERMINAL_ARRIVALS_BY_RATING: [
    { from: 0.0, perMinute: 1 },
    { from: 3.0, perMinute: 2 },
    { from: 3.5, perMinute: 3.5 },
    { from: 4.0, perMinute: 5 },
    { from: 4.3, perMinute: 8 },
    { from: 4.6, perMinute: 11 },
  ],
...
const APRON_ECONOMY = {
  ...
  // Обслуживание вертолёта на площадке равно интервалу его прилёта: тогда
  // одно место обслуживает ровно один договор. Раньше было 30 к 2, то есть
  // место тянуло полтора десятка договоров — площадки и их апгрейды не имели
  // смысла, а очередь не могла возникнуть в принципе.
  HELI_STAND_MINUTES: 12,
  // Вертолётный интервал отдельный: самолётный (30 мин) — опорное число, от
  // которого посчитаны квоты ВПП и потолки самолётных договоров.
  HELI_ARRIVAL_INTERVAL: 12,
...
const CONTRACT_ECONOMY = {
  ...
  BASE_PAY_PER_ARRIVAL: 55,   // было 105; прилёты стали чаще, доход выровнен
  // Вертолётные прилёты теперь в 2.5 раза чаще (интервал 30 -> 12), поэтому
  // плата за один прилёт ниже: иначе доход ветки подскочил бы втрое.
  ...
  MAX_ACTIVE_BASE: 1,
  MAX_ACTIVE_OVERBOOK: 1.4,
  // Вертолётный потолок: одно место = один договор, плюс 20% перегрузки,
  // чтобы очередь могла возникнуть как сигнал. Не больше двенадцати —
  // дальше управлять договорами становится утомительно.
  HELI_OVERBOOK: 1.2,
  HELI_MAX: 12,   // для самолётов: насколько потолок выше пропускной способности
  // Вертолёты: одно место = один договор, плюс 20% запаса, чтобы перегрузка
  // (и очередь как сигнал) была достижима. Больше 12 договоров держать
  // незачем — столько мест всё равно не построить.
  HELI_OVERBOOK: 1.2,
  HELI_MAX_ACTIVE: 12,
...
const BUILD_LIMITS = {
  // Три площадки по пять уровней — пятнадцать мест. Больше при потолке в
  // двенадцать договоров не нужно никогда: прежние пять площадок давали
  // двадцать пять мест, десять из которых были мертвы.
  helipad: 3,          // вертолётные площадки
  // ВПП без лимита: сколько полос строить, решает сам игрок. Естественный
...
// Самолёты не занимают отдельные постройки — им нужны "места" (слоты): стоянка = 1 слот,
...
const AIRCRAFT_EVENTS = {
  ...
  // Новостные события включаются только с этого уровня аэропорта: раньше
  // игроку не до происшествий с телетрапом и служебными собаками.
  MIN_LEVEL: 5,
  // --- Ласточка в телетрапе: без последствий, просто добрая новость ---
  // --- Служебная собака нашла наркодилера: премия опытом, случайно ---
  DRUG_DOG_XP_MIN: 1000,
  DRUG_DOG_XP_MAX: 5000,
  // --- Сломались шлагбаумы на въезде: штраф ---
  BARRIER_FINE: 20000,
